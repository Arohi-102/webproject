...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("UPDATE `users` SET email = ?, contact_number = ?, user_name = ?, user_type = ?, address = ? WHERE user_id = ?");
    ...
    $stmt->bind_param("sssssi", $email, $contactNumber, $username, $userType, $address, $userID);
    ...
    if ($stmt->execute()) {
        ...
        echo "<script>
                alert('Error: Unable to update user data. Please try again!');
                window.history.back();
              </script>";
    }
}
